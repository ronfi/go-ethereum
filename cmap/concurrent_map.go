...
// this is a copy from https://github.com/orcaman/concurrent-map
...
// RemoveIf removes an element from the map if exists.
func (m ConcurrentMap) RemoveIf(key string) (interface{}, bool) {
	// Try to get shard.
	shard := m.GetShard(key)
	shard.RLock()
	v, ok := shard.items[key]
